...
public class AssembleResult
{
	...
	void assembleBlock(LocatedBlock lb)
	{
		...
		xOffset = Math.min(xOffset, lb.coords.getX());
		yOffset = Math.min(yOffset, lb.coords.getY());
		zOffset = Math.min(zOffset, lb.coords.getZ());
	}
	...
	public EntityShip getEntity(World world)
	{
		...
		//entity.setPilotSeat(shipMarkingBlock..blockMeta & 3, shipMarkingBlock.coords.getX() - xOffset, shipMarkingBlock.coords.getY() - yOffset, shipMarkingBlock.coords.getZ() - zOffset);
		...
		try
		{
			...
			{
				ix = lb.coords.getX() - xOffset;
				iy = lb.coords.getY() - yOffset;
				iz = lb.coords.getZ() - zOffset;
	...
	public void checkConsistent(World world)
	{
		...
		for (LocatedBlock lb : assembledBlocks)
		{
			IBlockState blockState = world.getBlockState( lb.coords );
