...
public class TileEntityGaugeRenderer extends TileEntitySpecialRenderer
{
	...
	public void renderGauge(TileEntityGauge tileentity, double x, double y, double z, float partialticks)
	{
		...
		boolean extended = (tileentity.getBlockMetadata() & 4) != 0;
		...
		{
			dx = tileentity.parentShip.seatX - tileentity.getPos().getX();
			dy = tileentity.parentShip.seatY - tileentity.getPos().getY();
			dz = tileentity.parentShip.seatZ - tileentity.getPos().getZ();
		} else
		{
			dx = tileentity.parentShip.posX - renderManager.viewerPosX;
			dy = tileentity.parentShip.posY - renderManager.viewerPosY;
			dz = tileentity.parentShip.posZ - renderManager.viewerPosZ;
		}
