...
public class EntityShip extends EntityBoat implements IEntityAdditionalSpawnData
{
	...
	public void setRotatedBoundingBox()
	{
		...
		{
			setEntityBoundingBox(new AxisAlignedBB( posX - shipChunk.getCenterX(), posY, posZ - shipChunk.getCenterZ(), posX + shipChunk.getCenterX(), posY + height, posZ + shipChunk.getCenterZ() ) );
	...
	*/
	...
	protected void handleServerUpdate(double horvel)
	{
		...
		for (; belowwater < bpermeter; belowwater++)
		{
			...
			setEntityBoundingBox(new AxisAlignedBB(getBoundingBox().minX, d1, getBoundingBox().minZ, getBoundingBox().maxX, d2, getBoundingBox().maxZ));
		...
		double maxvel = ShipWright.instance.modConfig.speedLimit;
		if (newhorvel > maxvel)
		{
		...
		if (riddenByEntity instanceof EntityLivingBase)
		{
			...
			if (ShipWright.instance.modConfig.shipControlType == ModSettings.CONTROL_TYPE_ARCHIMEDES)
			{
				...
				*/
				...
				motionX = vec.xCoord;
				motionZ = vec.zCoord;
			} else if (ShipWright.instance.modConfig.shipControlType == ModSettings.CONTROL_TYPE_VANILLA)
			{
				if (throttle > 0.0D)
				{
	...
	protected void spawnParticles(double horvel)
	{
		...
		if (capabilities.getEngines() != null)
		{
			...
			for (TileEntityEngine engine : capabilities.getEngines())
			{
				if (engine.isRunning())
				{
					Vec3 vec = new Vec3(engine.getPos().getX() - shipChunk.getCenterX() + 0.5f, engine.getPos().getY(), engine.getPos().getZ() - shipChunk.getCenterZ() + 0.5f);
	...
	public void updateRiderPosition(Entity entity, BlockPos pos, int flags)
	{
		if (entity != null)
		{
			...
			if ((flags & 1) == 1)
			{
				...
				IBlockState blockState = shipChunk.getBlockState(new BlockPos(x1, MathHelper.floor_double(y1 + getMountedYOffset() + entity.getYOffset()), z1));
				Block block = blockState.getBlock();
	...
	*/
	...
	public void driftToGrid()
	{
		...
		if( Math.abs( motionX ) < BASE_FORWARD_SPEED * 0.25f && Math.abs( motionZ ) < BASE_FORWARD_SPEED * 0.25f )
		{
			...
			Vec3 target = new Vec3(getBlockAt(posX, size.xCoord), getBlockAt(posY, size.yCoord), getBlockAt(posZ, size.zCoord));
		...
		for (int i = shipChunk.minX(); i < shipChunk.maxX(); i++)
		{
			for (int j = shipChunk.minY(); j < shipChunk.maxY(); j++)
			{
				for (int k = shipChunk.minZ(); k < shipChunk.maxZ(); k++)
				{
					...
					if (block != Blocks.air)
					{
						block.dropBlockAsItem(worldObj, new BlockPos( MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ) ), blockState, 0);
					}
				}
			}
		}
