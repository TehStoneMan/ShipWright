...
public class MetaRotations
{
	...
	public IBlockState getRotatedMeta(Block block, IBlockState blockSate, int rotate)
	{
		if (rotate == 0) return blockSate;
		BlockMetaRotation rotation = metaRotationMap.get(Block.getIdFromBlock(block));
		if (rotation == null) return blockSate;
		return rotation.getRotatedMeta(blockSate, rotate);
	}
	...
	public void addMetaRotation(Block block, int bitmask, int... metarotation)
	{
		if (block == null)
		{
			LogManager.getLogger( ModInfo.MODID ).error("Adding null block meta rotations");
			return;
		}
		LogManager.getLogger( ModInfo.MODID ).trace("Adding meta rotations (block=" + Block.blockRegistry.getNameForObject(block) + ", id=" + Block.getIdFromBlock(block) + ", mask=" + bitmask + ", rot=" + Arrays.toString(metarotation) + ")");
	...
	{
		metaRotationsDirectory = new File(configdirectory, "ShipWright");
	...
	{
		LogManager.getLogger( ModInfo.MODID ).warn("Error in metarotation file at line " + lineno + " (" + msg + ")");
	}
	...
	public void readMetaRotationFiles()
	{
		...
		try
		{
			...
			{
				LogManager.getLogger( ModInfo.MODID ).info("Outdated default.mrot detected: " + ome.getLocalizedMessage());
				createDefaultMrot();
				readMetaRotationFile(new File(metaRotationsDirectory, "default.mrot"));
			} catch (FileNotFoundException fnfe)
			{
				LogManager.getLogger( ModInfo.MODID ).info("default.mrot file not found: " + fnfe.getLocalizedMessage());
				createDefaultMrot();
				readMetaRotationFile(new File(metaRotationsDirectory, "default.mrot"));
			} catch (Exception e0)
			{
		...
		{
			LogManager.getLogger( ModInfo.MODID ).error("Could not load default meta rotations", e1);
		}
	...
	{
		LogManager.getLogger( ModInfo.MODID ).info("Reading metarotation file: " + file.getName());
	...
	{
		LogManager.getLogger( ModInfo.MODID ).info("Creating default.mrot");
