...
public class ModSettings
{
	...
	{
		final Block[] defaultBlockBlacklist = { Blocks.dirt,
												Blocks.grass,
												Blocks.sand,
												Blocks.gravel,
												Blocks.clay,
		...
		final Block[] defaultBlocksWhitelist = { Blocks.acacia_door,
												 Blocks.acacia_fence,
		...
		final Block[] defaultOverwritableBlocks = { Blocks.tallgrass, Blocks.waterlily };

		final String[] defaultMaterialDensities = { "\"minecraft:air=0.0\"", "\"minecraft:wool=0.1\"" };
		final String[] defaultBlockDensities = { "\"shipwright:floater=0.04\"", "\"shipwright:balloon=0.02\"" };

		final String[] blockblacklistnames = new String[defaultBlockBlacklist.length];
		for( int i = 0; i < defaultBlockBlacklist.length; i++ )
			blockblacklistnames[i] = String.valueOf( Block.blockRegistry.getNameForObject( defaultBlockBlacklist[i] ) );

		final String[] blockwhitelistnames = new String[6 + defaultBlocksWhitelist.length];
		for( int i = 0; i < blockwhitelistnames.length - 6; i++ )
			blockwhitelistnames[i] = String.valueOf( Block.blockRegistry.getNameForObject( defaultBlocksWhitelist[i] ) );
		...
		final String[] overwritableblocksnames = new String[defaultOverwritableBlocks.length];
		for( int i = 0; i < defaultOverwritableBlocks.length; i++ )
			overwritableblocksnames[i] = String.valueOf(Block.blockRegistry.getNameForObject( defaultOverwritableBlocks[i] ));
		...
		final String[] forbiddenblocks = config.get( "mobile_chunk", "forbidden_blocks", blockblacklistnames, "A list of blocks that will not be added to a ship." ).getStringList();
		final String[] allowedblocks = config.get( "mobile_chunk", "allowed_blocks", blockwhitelistnames, "A list of blocks that are allowed on a ship." ).getStringList();
		final String[] overwritableblocks = config.get( "mobile_chunk", "overwritable_blocks", overwritableblocksnames, "A list of blocks that may be overwritten when decompiling a ship." ).getStringList();
	...
	public void postLoad()
	{
		...
		for( int i = 0; i < loadedBlockDensities.length; i++ )
		{
			...
			s = s.replace( "\"", "" );
			final String[] pair = splitpattern.split( s );
			if( pair.length != 2 )
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "Invalid key-value pair at block_densities[" + i + "]" );
				continue;
			}
			...
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "Cannot parse value " + pair[1] + " to floating point at block_densities[" + i + "]" );
				continue;
			}
			final Block block = Block.getBlockFromName( key );
			if( block == null )
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "No block found for " + key + " at block_densities[" + i + "]" );
				continue;
			}
		...
		for( int i = 0; i < loadedMaterialDensities.length; i++ )
		{
			...
			if( pair.length != 2 )
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "Invalid key-value pair at material_densities[" + i + "]" );
				continue;
			}
			final String key = pair[0];
			float density;
			...
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "Cannot parse value " + pair[1] + " to floating point at material_densities[" + i + "]" );
				continue;
			}
			final Block block = Block.getBlockFromName( key );
			if( block == null )
			{
				LogManager.getLogger( ModInfo.MODID ).warn( "No block found for " + key + " at material_densities[" + i + "]" );
				continue;
			}
	...
	{
		final String id = String.valueOf( Block.blockRegistry.getNameForObject( block ) );
